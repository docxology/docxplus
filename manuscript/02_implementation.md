...
{{ZIP_EPOCH}}, `[Content_Types].xml` is written first, and the remaining parts follow in lexicographic order.
...
using $\text{AES-{{KEY_BITS}}-GCM}$. The module slot name $s$ is bound as Additional Authenticated Data (AAD):
...
- **Argon2id** (RFC 9106 recommended profile) [@rfc9106] — {{ARGON2_MEMORY_MIB}} MiB memory cost, time cost
  $t={{ARGON2_TIME_COST}}$, parallelism $p={{ARGON2_PARALLELISM}}$.
- **Scrypt** (the default) [@rfc7914] — $N={{SCRYPT_N}}$ ($2^{{{SCRYPT_N_LOG2}}}$), $r={{SCRYPT_R}}$,
  $p={{SCRYPT_P}}$: memory-hard at negligible interactive cost.
- **PBKDF2-HMAC-SHA512** (compatibility) — {{PBKDF2_ITERATIONS}} iterations, per OWASP 2023 guidance, for
...
$128 \cdot N \cdot r \le {{MAX_SCRYPT_MEMORY_MIB}}$ MiB, the same ceiling Argon2id already enforced.
...
{{CLI_COMMAND_COUNT}} commands: {{CLI_COMMANDS}}.
