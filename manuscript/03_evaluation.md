...
All {{DOSSIER_MODULE_COUNT}} modules extract and verify:
...
The harness comprises {{TEST_COUNT}} test functions under a {{COVERAGE_GATE}}% coverage gate, with no mocks
...
Over a {{RT_FILES}}-file, {{RT_DIRS}}-directory tree, all {{RT_PASSED}} of {{RT_CHECKS}} invariants hold.
Both profiles validate against their own conformance rules; both verify provenance under a pinned key and
refuse a wrong one; both extract {{RT_COMPARED}} files identical to the originals; both carry and
cryptographically verify a reproduction attestation. The sealed `.docx` is {{RT_DOCX_BYTES}} bytes and the
`.odt` {{RT_ODT_BYTES}} bytes, and the packed project payload is *byte-identical between them* — parity that
...
![What bounds each channel, plotted against the axis on which they actually differ. `metadata` has a hard ceiling fixed by the format and flat in carrier size; `stego_media` scales with carrier area and overtakes that ceiling at roughly {{STEGO_CROSSOVER_PX}} pixels a side; the remaining three have no channel-imposed limit at all and are drawn as an open region rather than given an invented number. The marked points are measured rather than computed: a payload of exactly that size was embedded into a real carrier and read back before the point was plotted.](../output/figures/capacity.png){#fig:capacity width=85%}
...
- **`metadata`** is deliberately small, bounded at {{METADATA_MAX_BYTES}} bytes in `docProps/custom.xml`:
...
- **`stego_media`** scales with carrier resolution under 1-bit LSB encoding: a $256\times 256$ RGB PNG holds
  {{MEDIA_CAP_256}} bytes, a $512\times 512$ PNG holds {{MEDIA_CAP_512}}. Under `redundancy=N` the payload is
...
{{REDTEAM_AUDIT_COUNT}} cycles of adversarial review closing {{REDTEAM_FINDING_COUNT}} confirmed findings.
