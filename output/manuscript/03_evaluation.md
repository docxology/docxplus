...
The harness comprises 467 test functions under a 90% coverage gate, with no mocks
...
cryptographically verify a reproduction attestation. The sealed `.docx` is 4,563 bytes and the
`.odt` 3,643 bytes, and the packed project payload is *byte-identical between them* — parity that
